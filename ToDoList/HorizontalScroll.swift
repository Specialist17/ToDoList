...
open class HorizontalScroll: UICollectionViewFlowLayout {
    
    fileprivate var lastCollectionViewSize: CGSize = CGSize.zero
    // transition animation
    open var scalingOffset: CGFloat = 200
    
    // scale for the collection item
    open var minimumScaleFactor: CGFloat = 0.5
    
    
    open var scaleItems: Bool = false
    
    static func configureLayout(_ collectionView: UICollectionView, itemSize: CGSize, minimumLineSpacing: CGFloat) -> HorizontalScroll {
        
        let layout = HorizontalScroll()
